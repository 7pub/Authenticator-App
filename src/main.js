...
module.exports = {
    ...
    build: (secret, account, issuer, period) => {
        ...
        let result = 'otpauth://totp/' 
        //+ 'Di%C3%B6zese%20Linz%3A' 
...
console.log(toggleOtpAuthDetails);
// CONCATENATED MODULE: ./index.js


document.getElementById('app-version').innerText = {"version":"2.0.4-e824c035da99a9ad1e70be304c81f7dc798bd3e9"}.version;
const QRCode = __webpack_require__(0);
const TOTP = __webpack_require__(1);
const Cookies = __webpack_require__(3);
const OTPAuthUrl = __webpack_require__(4);
function copyToClipboard(value) {
    const input = document.createElement("input");//Create a temporary input
    document.body.appendChild(input);//Append it to body
    input.setAttribute("value", value);//Set input value
    input.select();//Select input value
    document.execCommand("copy"); //Copy input value
    document.body.removeChild(input); //Remove input from body
}
...
function updateQrCode() {
    ...
    if (secret && account) {
        ...
        qrImage.makeCode('@dioezese-linz.at');
